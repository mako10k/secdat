...
static void secdat_fuse_print_usage(const char *program_name, FILE *stream)
{
    fprintf(
        ...
        _("Usage: %s [OPTIONS] MOUNTPOINT [-- CMD [ARGS...]]\n"
          ...
          "      --bulk-gate     include only keys allowed for bulk-gated selection\n"
...
)
{
    ...
    fputs(",\n  \"bulk_gate\": ", stdout);
    fputs(state->filters.bulk_gate ? "true" : "false", stdout);
...
static int secdat_fuse_parse_args(int argc, char **argv, struct secdat_fuse_state *state)
{
    static const struct option long_options[] = {
        ...
        {"bulk-gate", no_argument, NULL, 1001},
    ...
    while ((option = getopt_long(argc, argv, "+:d:s:p:x:fhV", long_options, NULL)) != -1) {
        ...
        case 'x':
            ...
            break;
        case 1001:
            state->filters.bulk_gate = 1;
            break;
        case 9003:
            return secdat_fuse_reject_removed_legacy_flag("--inject-bulk-gate", "--bulk-gate");
        case 1007:
            return secdat_fuse_reject_removed_legacy_flag("--sandbox-injectable", "--bulk-gate");
        case 1002:
            state->dry_run = 1;
            break;
        case 1003:
