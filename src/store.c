...
static int secdat_v2_secret_objects_dir(const char *domain_id, const char *store_name, char *buffer, size_t size)
{
    ...
    if (secdat_store_root(domain_id, store_name, store_root, sizeof(store_root)) != 0
        || secdat_join_path(objects_dir, sizeof(objects_dir), store_root, "objects") != 0) {
        return 1;
    }
...
static int secdat_read_v2_text_file(const char *path, const char *magic, char **text_out, size_t *length_out)
{
    ...
    if (length > 16384 || memchr(data, '\0', length) != NULL) {
        secdat_secure_clear(data, length);
        free(data);
        return 1;
    }
...
static int secdat_read_v2_domain_entry_info(const char *path, const char *file_entry_id, struct secdat_v2_domain_entry_info *info)
{
    ...
    valid = seen_entry_id && seen_secret_id && seen_key_visibility && seen_entry_inject
        && (info->key_visibility != SECDAT_KEY_VISIBILITY_ALWAYS || info->has_key);
...
static int secdat_read_v2_secret_object_info(const char *path, const char *file_secret_id, struct secdat_v2_secret_object_info *info)
{
    ...
    if (!secdat_uuid_is_valid(file_secret_id)
        || secdat_read_v2_text_file(path, secdat_v2_secret_object_magic, &text, &length) != 0) {
        return 1;
    }
...
static int secdat_collect_v2_visible_keys(const char *domain_id, const char *store_name, struct secdat_key_list *keys)
{
    ...
    for (index = 0; index < entry_ids.count; index += 1) {
        ...
        if (info.key_visibility == SECDAT_KEY_VISIBILITY_ALWAYS && info.has_key
            && secdat_key_list_append(keys, info.key) != 0) {
            goto cleanup;
        }
    }
...
)
{
    ...
    for (index = 0; index < entry_ids.count; index += 1) {
        ...
        if (info->key_visibility == SECDAT_KEY_VISIBILITY_ALWAYS && info->has_key && strcmp(info->key, key) == 0) {
            found = 1;
            break;
        }
    }
...
static size_t secdat_v2_count_secret_references(const struct secdat_key_list *references, const char *secret_id)
{
    size_t index;
    size_t count = 0;

    for (index = 0; index < references->count; index += 1) {
        if (strcmp(references->items[index], secret_id) == 0) {
            count += 1;
        }
    }
    return count;
...
)
{
    ...
    if (secdat_remove_if_exists(tombstone_path) != 0
        || secdat_atomic_write_file(value_path, encrypted, encrypted_length) != 0
...
)
{
    ...
    if (options->orphaned) {
        for (index = 0; index < valid_objects.count; index += 1) {
            if (secdat_v2_count_secret_references(&references, valid_objects.items[index]) == 0) {
    ...
    if (options->refcount) {
        for (index = 0; index < valid_objects.count; index += 1) {
            ...
            actual_refcount = secdat_v2_count_secret_references(&references, valid_objects.items[index]);
...
cleanup:
    ...
    secdat_key_list_free(&references);
    return status;
}
...
static int secdat_local_key_exists_in_store(const char *domain_id, const char *store_name, const char *key, int *exists)
{
    ...
    *exists = 0;
    ...
    if (format == SECDAT_STORE_FORMAT_V2) {
        lookup_status = secdat_lookup_v2_domain_entry(domain_id, store_name, key, &v2_entry, entry_path, sizeof(entry_path));
        if (lookup_status > 1) {
            return 1;
        }
...
static int secdat_count_v2_secret_references_in_store(
    const char *domain_id,
    const char *store_name,
    const char *secret_id,
    size_t *count
...
)
{
    ...
    attrs.sandbox_inject = object.secret_inject == SECDAT_SANDBOX_INJECT_NEVER
        ? SECDAT_SANDBOX_INJECT_NEVER
        : SECDAT_SANDBOX_INJECT_ALLOW;
    return secdat_write_v2_secret_object_file(secret_objects_dir, secret_id, &attrs, 1, refcount, object_path, sizeof(object_path));
}
...
static int secdat_remove_v2_local_key(const char *domain_id, const char *store_name, const char *key, int *removed)
{
    ...
    *removed = 0;
    ...
    if (object_is_valid) {
        ...
        } else if (secdat_update_v2_secret_refcount(domain_id, store_name, entry.secret_id, references - 1) != 0) {
            return 1;
        }
    }
