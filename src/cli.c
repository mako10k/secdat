...
int secdat_cli_complete(int argc, char **argv)
{
    ...
    static const char *const load_options[] = {
        "--conflict", "--allow-link-split", "--mask-action", "--mask-warnings", "--warn-mask", "--no-warn-mask",
        "--dry-run", "--json", "--help", "-h", NULL,
    };
...
static void secdat_cli_print_usage_line(const char *program_name, enum secdat_command_type command)
{
    ...
    case SECDAT_COMMAND_LOAD:
        secdat_cli_print_usage_columns(program_name, "[-d DIR|--dir DIR] [-s STORE|--store STORE]", "load", "[--conflict=reject|overwrite|skip] [--allow-link-split] [--mask-action=preserve|reject] [--mask-warnings=default|on|off] [--warn-mask|--no-warn-mask] [--dry-run] [--json] FILE");
        break;
...
static void secdat_cli_print_target_meaning(const char *target)
{
    ...
    if (target != NULL && strcmp(target, "load") == 0) {
        ...
        secdat_cli_print_detail_line(_("  linked v2 objects require a v2 destination; --allow-link-split explicitly materializes independent v1 values\n"));
