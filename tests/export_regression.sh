...
rc, stdout, stderr = run([
    ...
    "exec",
    "--inject", "secret:only=CONTROL_*",
    "--inject", "secret:require=CONTROL_TOKEN",
    "python3",
    "-c",
    "import os,sys; sys.stdout.write(repr(os.environ['CONTROL_TOKEN']))",
])
...
rc, stdout, stderr = run([
    ...
    "exec",
    "--inject", "secret:only=CONTROL_*",
    "--inject", "secret:require=ROOT_TOKEN",
    "python3",
    "-c",
...
rc, stdout, stderr = run([
    ...
    str(child_dir),
    "exec",
    "--inject", r"secret:rename=/^MY_REDMINE_.\+/s/MY_REDMINE_\(.\+\)/SPV_REDMINE_\1/",
    "python3",
    "-c",
...
rc, stdout, stderr = run([
    ...
    str(child_dir),
    "exec",
    "--inject", "secret:only=MY_REDMINE_*",
    "--inject", r"secret:rename=s/^MY_REDMINE_\(.\+\)$/SPV_REDMINE_\1/",
    "--dry-run",
    "python3",
    "-c",
...
rc, stdout, stderr = run([
    ...
    str(child_dir),
    "exec",
    "--inject", r"secret:rename=s/^MY_REDMINE_PROJECT$/BAD-NAME/",
    "--dry-run",
    "--json",
    "python3",
    "-c",
...
rc, stdout, stderr = run([
    ...
    str(child_dir),
    "exec",
    "--inject", r"secret:rename=s/^MY_REDMINE_.*$/DUP_REDMINE/",
    "--json-summary",
    "python3",
    "-c",
    "print('unreachable')",
...
for expression, expected in [
    ...
    rc, stdout, stderr = run([
        ...
        str(child_dir),
        "exec",
        "--inject", f"secret:rename={expression}",
        "python3",
        "-c",
...
rc, stdout, stderr = run([
    ...
    str(child_dir),
    "exec",
    "--inject", r"secret:rename=s/^MY_REDMINE_PROJECT$//",
    "python3",
    "-c",
    "print('unreachable')",
])
...
if rc == 0 or "invalid environment variable name from secret rename: BAD-NAME" not in stderr:
    fail(f"invalid secret rename result unexpectedly succeeded: rc={rc} stdout={stdout!r} stderr={stderr!r}")
