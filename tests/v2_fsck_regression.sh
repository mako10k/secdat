...
if rc != 0 or stdout != "APP_TOKEN\tkey_visibility=always\tvalue_access=unlocked\tbulk_select=named\n" or stderr != "":
...
if rc != 0 or stdout != "" or stderr != "":
    fail(f"explicit v2 key should be excluded from bulk bulk-gate list: rc={rc} stdout={stdout!r} stderr={stderr!r}")

rc, stdout, stderr = run([bin_path, "--dir", str(domain), "attr", "APP_TOKEN", "--bulk-select", "exclude"])
if rc != 0 or stdout != "" or stderr != "":
...
if "bulk_select_value=include\n" not in object_text or "refcount=1\n" not in object_text:
    fail("clean v2 attr did not preserve secret object metadata")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "attr", "APP_TOKEN", "--bulk-select", "include"])
if rc != 0 or stdout != "" or stderr != "":
...
object_text = read_object_header_text(secret_objects_dir / f"{secret_id}.sec")
(secret_objects_dir / f"{secret_id}.sec").write_text(object_text.replace("bulk_select_value=include\n", "bulk_select_value=exclude\n"), encoding="utf-8")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "attr", "APP_TOKEN", "--bulk-select", "include"])
if rc == 0 or "secret object excludes bulk select: APP_TOKEN" not in stderr:
...
rc, stdout, stderr = run([
    bin_path, "--dir", str(domain), "exec", "--bulk-gate",
...
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "export", "--bulk-gate"])
if rc != 0 or stdout != "" or stderr != "":
...
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "set", "APP_SECRET", "--value", "secret-value", "--bulk-select", "named"])
if rc != 0 or stdout != "" or stderr != "":
...
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "set", "APP_BULK", "--value", "bulk-value", "--bulk-select", "include"])
if rc != 0 or stdout != "" or stderr != "":
    fail(f"pure v2 set bulk injectable failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "set", "APP_EXPLICIT", "--value", "explicit-value", "--bulk-select", "named"])
if rc != 0 or stdout != "" or stderr != "":
...
if rc != 0 or not exec_stderr_ok(stderr):
    fail(f"pure v2 bulk-gate exec failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
if json.loads(stdout) != {"APP_BULK": "bulk-value"}:
    fail(f"pure v2 bulk-gate exec payload mismatch: {stdout!r}")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "export", "--bulk-gate", "--pattern", "APP_*"])
if rc != 0 or stderr != "":
    fail(f"pure v2 bulk-gate export failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
...
for unexpected in ["APP_EXPLICIT", "APP_SECRET"]:
    if unexpected in stdout:
        fail(f"pure v2 bulk-gate export unexpectedly included {unexpected}: {stdout!r}")
...
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "attr", "APP_SECRET", "--bulk-select", "include"])
if rc != 0 or stdout != "" or stderr != "":
    fail(f"pure v2 linked source attr bulk failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "attr", "APP_SECRET_LINK", "--bulk-select", "exclude"])
if rc != 0 or stdout != "" or stderr != "":
    fail(f"pure v2 linked target attr never failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "ls", "--bulk-gate", "--pattern", "APP_SECRET*"])
if rc != 0 or stderr != "":
    fail(f"pure v2 linked bulk-select attr isolation ls failed: rc={rc} stdout={stdout!r} stderr={stderr!r}")
if stdout != "APP_SECRET\n":
    fail(f"pure v2 linked bulk-select attr should remain per entry: {stdout!r}")
rc, stdout, stderr = run([bin_path, "--dir", str(domain), "set", "APP_SECRET_LINK", "--value", "linked-value", "--bulk-select", "named"])
if rc != 0 or stdout != "" or stderr != "":
...
rc, stdout, stderr = run([bin_path, "--dir", str(consumer_domain), "set", "REMOTE_SECRET", "--value", "remote-linked-value", "--bulk-select", "named"])
if rc != 0 or stdout != "" or stderr != "":
...
rc, stdout, stderr = run([
    ...
    "--key-visibility", "unlocked", "--value", "hidden-value", "--bulk-select", "named",
])
if rc != 0 or stdout != "" or stderr != "":
...
assert_contains(stdout, "HIDDEN_TOKEN\tkey_visibility=unlocked\tvalue_access=unlocked\tbulk_select=named\n", "hidden key metadata")
