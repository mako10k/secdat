...
`fsck --format v2 --dependency-index` validates the global hash-committed reverse dependency index against every registered mask and relation primary. Missing, building, corrupt, or stale coverage fails closed. Add `--repair` to durably publish `building`, scan and verify all M/R/D dependency spaces, write immutable `SECDATDEPNODE1` nodes, and finally publish a complete `SECDATDEPSTATE1` root. Existing mask and relation writers invalidate a complete generation before changing a primary; rebuild explicitly before a reverse-dependent hidden-name operation. Exact R lookup rejects a relation-dependent visible-to-hidden transition. Until atomic mask-primary rewriting is routed, an M dependency rejects either key-visibility transition rather than leaving a mask with stale visibility/name representation; updates that keep the existing visibility remain allowed. Index records contain identity/hash addresses, never plaintext hidden key names.
