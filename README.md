...
The v1 storage layout keeps key names visible on disk, so `key_visibility=unlocked` remains reserved and is rejected there. The v2 graph layout reads and writes split domain-entry/secret-object attributes for visible keys, including object-owned `value_access` rewrites; hidden-key updates are still part of the remaining v2 write path. `value_access=unlocked` is the normal encrypted-at-rest mode; `value_access=always` is the public/plaintext-at-rest mode used by `--unsafe` and `--public-value`. `sandbox_inject` marks whether a future sandbox import flow may include the key: `never` excludes it, `explicit` allows only explicit key selection, and `allow` also allows pattern-based selection.
...
For stores marked as v2, `ls`, `exists`, `attr`, `set`, `get`, and `id KEYREF` use the v2 domain-entry/object graph for visible keys. `id` prints the resolved `secret_id` without reading the value. Migrated stores keep their preserved v1 value files as a fallback until a value is rewritten into v2 object-owned storage.
