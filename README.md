...
An ephemeral value lives only in the active domain-scoped session agent. It is available through normal `get`, `ls`, `exec`, SDK, and FUSE read paths, and it shadows a persisted same-name value or tombstone until explicitly removed. Effective enumeration makes the same overlay-first decision as direct lookup, so it does not expose or authenticate a shadowed v2 backing name with an unrelated process key; any unshadowed hidden backing entry still fails closed when that key cannot decrypt it. Its key and value are both unlocked-only; `bulk_select` defaults to `exclude`, or may be set to `include` for bulk-gated selection. Refreshing the same session with the same master key preserves it. `lock`, expiry, or replacing the session with a different master key clears it. A process-level `SECDAT_MASTER_KEY` does not hide an active ephemeral value, but that environment variable alone cannot create one because no session agent owns its lifetime.
