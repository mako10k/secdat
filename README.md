...
For stores marked as v2, `ls`, `exists`, `attr`, and `id KEYREF` read the v2 domain-entry/object graph. `id` prints the resolved `secret_id` without reading the value. `get` remains compatible with migrated stores through the preserved v1 value files until v2 object-owned value storage is implemented.
