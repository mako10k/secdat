...
When built with `--enable-fuse`, `secdat-fuse` mounts selected keys as read-only files through FUSE 3. It uses the SDK list/get path instead of store internals, accepts the same domain/store context options, supports repeated `--pattern` and `--pattern-exclude` filters, and supports `--sandbox-injectable`. `--require-key KEY` may be repeated to refuse dry-runs and mounts unless each required key remains selected after filters. `--dry-run MOUNTPOINT` prints the file names that would be exposed without mounting or reading secret values; add `--json` for machine-readable preflight output. `secdat-fuse ... MOUNTPOINT -- CMD [ARGS...]` mounts in a child process, runs the command, unmounts with `fusermount3 -u`, and returns the command status unless cleanup itself fails after a successful command. File metadata reports size 0 by default so `getattr` does not read secret values; opt in to `--size-metadata` only when consumers need real file sizes and the extra plaintext/decryption exposure from stat-like operations is acceptable. A real mount exposes plaintext through normal file reads, so use a private mountpoint, keep `allow_other` disabled, and unmount as soon as the consuming process no longer needs the files.
