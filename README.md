...
The planned storage v2 moves from one domain-local file per key to a directory/inode-like split between domain entries and secret objects. Domain entries own key names and key visibility; secret objects own values and value access. The current v2 path includes linked secrets (`ln`) across v2 domains/stores, secret UUID references, object address metadata, object-key encrypted value payloads for new or rewritten encrypted v2 values, refcount/orphan checks, and a migration-first compatibility path from the current v1 store. Direct `ln --secret-id UUID` remains planned and still requires explicit authorization semantics. See [docs/secdat-spec.md](docs/secdat-spec.md#510-planned-store-v2-domain-entries-and-secret-objects).
