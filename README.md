...
The planned storage v2 moves from one domain-local file per key to a directory/inode-like split between domain entries and secret objects. Domain entries own key names and key visibility; secret objects own values and value access. The current v2 path includes linked secrets (`ln`) across v2 domains/stores, secret UUID references, object address metadata, object-owned payloads for new or rewritten v2 values, refcount/orphan checks, and a migration-first compatibility path from the current v1 store. Direct source-object links such as planned `ln @UUID DST_KEYREF` remain planned and still require explicit authorization semantics; `@UUID` is a source operand, not a destination. See [docs/secdat-spec.md](docs/secdat-spec.md#510-planned-store-v2-domain-entries-and-secret-objects).
