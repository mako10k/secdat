_secdat_complete()
{
    ...
    commands=(help version ls exists get set rm mv cp exec export save load unlock passwd lock status store domain)
    ...
    while (( index < COMP_CWORD )); do
        case "${COMP_WORDS[index]}" in
            --dir|-d|--store|-s)
                ...
                continue
                ;;
            help|version|ls|exists|get|set|rm|mv|cp|exec|export|save|load|unlock|passwd|lock|status|store|domain)
    ...
    case "$cmd" in
        ...
        status)
            ...
            return 0
            ;;
        exists|save|load|unlock|passwd|lock|rm|mv|cp|version)
            return 0
            ;;
    esac
