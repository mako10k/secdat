...
- `unlock --readonly` reuses an existing master key but must reject mutating commands, including `gc` without `--dry-run` and `store finalize-migration` without `--dry-run`, while keeping reads, listing, export-like operations, dry-run cleanup review, and status available
