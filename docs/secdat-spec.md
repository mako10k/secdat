...
- `--inject-gate=sandbox` applies the bulk `sandbox_inject` store-attribute pre-filter before secret supply
...
- `--inject-gate=sandbox` keeps only keys whose effective `sandbox_inject` allows bulk selection
