...
- `secdat_sdk_write_at_preserve_attrs(options, keyref, value, value_length, offset, append)` and `secdat_sdk_resize_preserve_attrs(options, keyref, new_length)` hold the mutation lock across the effective-value read, update, and attribute-preserving write; an effective ephemeral target is rejected without changing the session agent or persisted store
...
- `secdat_sdk_relation_suggest_refresh(options, keyref, result_out)` returns structured rows equivalent to `relation suggest-refresh KEYREF`, with severity, relation id, leaked role, refresh role, refresh KEYREF, and reason; rows are derived from relation metadata and canonical KEYREFs without reading secret values
...
Returned list item arrays and SDK-owned buffers are allocated by `libsecdat` and owned by the caller; callers release `result_out->items` or returned buffers with `secdat_sdk_free()`. Relation-refresh row strings are stored inside the returned `result_out->items` allocation and are not freed separately. Redaction labels are static library-owned strings and do not need freeing. Metadata-only list APIs, exec plan APIs, redaction APIs, and relation-refresh APIs must not return plaintext secret values or decrypted value buffers.
...
secdat [--dir DIR] store finalize-migration STORE --from-format v1 [--dry-run]
...
- `--inject-file FILE` loads a YAML policy file; base file entries apply first, the one matched command profile can refine them, and CLI `--inject` options have the highest precedence
...
- an unlocked agent appends `proto=N` to the legacy-compatible `STATUS` fields; a client sends `CAPS` only after that advertisement and requires the returned version to match
- protocol v1 capabilities identify overlay lookup/list, per-key ephemeral operations, and lossless agent handover independently; a missing required capability is never inferred from an unknown command response
- v0.5.0 initiates automatic lossless handover only when a capable active agent advertises a lower protocol version; a different build advertising the same protocol is a known XFAIL tracked by issue #221, and forced handover tests are protocol coverage rather than trigger coverage
- there is no supported manual lossless same-protocol handover command; `SECDAT_TEST_FORCE_AGENT_HANDOVER` is test-only, while `lock` followed by `unlock` is a state-clearing restart that must not be represented as preserving volatile or per-key ephemeral state
- a pre-advertisement persistent agent may fall back to persisted-store reads when it returns `ERR invalid` or EOF for overlay lookup/list; a pre-advertisement volatile agent must fail closed because the persisted view cannot represent its effective state
- after `proto=N` is advertised, malformed status metadata, failed `CAPS`, a version mismatch, or an incomplete negotiated overlay response is a protocol error; status, unlock/lock transitions, readonly checks, and volatile/persisted mutation routing must not reinterpret it as an absent agent
- handover v1 is an in-memory agent-process protocol, not a persisted `store migrate`; its versioned binary snapshot contains the absolute expiry, configured duration, access mode, master key, and every overlay item, and each item contains its domain, store, key, entry/tombstone kind, save policy, unsafe flag, secret attributes, and length-delimited binary value
- handover snapshots end in a SHA-256 digest, reject duplicate `(domain, store, key)` identities and invalid enum/length combinations, and are bounded to 64 MiB and 65,536 items before allocation/import
- handover is accepted only from the same effective OS user; the old agent transfers its existing Unix listener descriptor with `SCM_RIGHTS`, retains authority while the successor validates the full snapshot, and commits through `READY`, `COMMIT`, `ACTIVATE`, and `ACTIVE` control messages
- each handover phase has a five-second monotonic deadline; failure before successor activation wipes the candidate import and leaves the old process, queued connections, socket path, state, and absolute expiry authoritative
- after successful activation the successor accepts on the transferred listener, the old process wipes its state and exits without unlinking the shared socket path, and no handover material is written to a store, bundle, wrapped-key file, or temporary file
- session-agent startup ownership is exclusive per domain through a persistent lock file: a contender rechecks the path after acquiring the lock, replaces only an unchanged non-socket stale entry or a socket whose unchanged device/inode identity returns `ECONNREFUSED`, preserves the pathname and fails closed on transient or indeterminate probe errors, and never lets the detached worker unlink an existing pathname before `bind`; the worker holds the lock through `bind`, `listen`, and readiness notification, and the parent accepts startup only through a five-second monotonic readiness handshake
- when readiness fails or times out, the worker must close its listener and unlink only the socket node identity that it bound; it must not remove a replacement socket or become an unreachable late agent
...
- `route:prefer=secret|ambient|error` resolves ambient/secret name collisions; per-name `route:NAME=...` rules may repeat; CLI route rules are matched before matched-profile route rules, then base policy-file route rules, and first match wins within each source
...
- `--inject-file FILE` loads YAML policies shaped like `docs/exec-injection-design.md` §7.4; `profiles.NAME.match.command` matches slash-qualified `CMD` exactly or unqualified `CMD` by basename, `match.argv_prefix` matches command arguments from the start, `profile_required: true` fails when no profile matches, and multiple matched profiles fail closed
- precedence is base file rules, then the matched command profile, then CLI `--inject`; route rules follow the same source order
...
domain/store/domain-ent/<entry-id>.dent
  public area:
    ...
    entry_id
    secret_id
    object_domain and object_store address fields (current transitional format)
    key_visibility
...
19. Add finalize/cleanup commands for legacy v1 fallback files only after v2 read/write, migration, fsck, gc, and rollback paths are covered by regression tests. This is implemented as `store finalize-migration STORE --from-format v1 [--dry-run]`; destructive cleanup refuses partial cleanup while blockers remain.
...
- require both entry-side and secret-side authorization for bulk-gated injection/export/list flows
